...
import Model.Date as Date exposing (Date, Month, Month(..), year)
...
compare (Interval intA) (Interval intB) =
    let
        start = Date.compare intA.start intB.start
        end = 
...
view interval =
    let
        ...
        getEndText =
            case newInterval.end of
                Nothing -> 
                    [text "Present"]
                Just x -> 
                    [Date.view x]
         
        hasLength : Interval -> Bool
        hasLength i = ((length i) /= Nothing)
        intervalLength = Maybe.withDefault (1,4) (length interval)

    in
    div [class "interval"] [
        div [class "interval-start"] [Date.view newInterval.start]
        , div [class "interval-end"] getEndText
        , p [classList [("interval-length", hasLength interval)]] [text (toString (Tuple.first intervalLength) ++ " years " ++ toString (Tuple.second intervalLength) ++ " months")]
    ]
